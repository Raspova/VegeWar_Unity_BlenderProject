...
using UnityEngine;
using static PlayerRts.state_t;    
...
//
    ...
    public PlayerRts ClosestEnemy()
    {
        List<PlayerRts> _enemyInView = enemyInView();
        float bestDistance = 9999.0f;
        PlayerRts bestCollider = null;
    
        foreach (PlayerRts enemy in _enemyInView)
        {
    ...
    [HideInInspector]
    public List<PlayerRts> _enemyInView = new List<PlayerRts>();
    
    public List<PlayerRts> enemyInView()
    {
        ...
        foreach (Collider unit in unitInView) {
            buff = unit.gameObject.GetComponent<PlayerRts>();//Map
    ...
    {
        PlayerRts nextTarget = ClosestEnemy();
    ...
    public void setTarget(PlayerRts _target)
    {
        if (_target == null)
            return;
        target = _target;
    ...
    private void OnDestroy() {
        ...
        if (UnitsManager.selectUnits.Contains(this))
            UnitsManager.selectUnits.Remove(this);
    }
