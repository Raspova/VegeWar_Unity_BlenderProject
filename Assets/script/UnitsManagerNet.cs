...
public class UnitsManagerNet : MonoBehaviour
{
    ...
    static public List<PlayerRtsNet> allUnits = new List<PlayerRtsNet>();
    static public List<PlayerRtsNet> selectUnits = new List<PlayerRtsNet>();
    Rect boxSelection;
    int UnitLayer;
    int FloorLayer;
    ...
    void initNUnits(GameObject unit, int n ,Vector3 pos, int teamId)
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            pos.x += 1;
            buff.GetComponentInChildren<PlayerRtsNet>().TeamId = teamId;
        }
    }
    void Start()
    {
    ...
    void deselectAll()
    {
        foreach (PlayerRtsNet unit in selectUnits)
            unit.changeSelect();
        selectUnits.Clear();
    }
    
    void attakTargetSelectedUnit(PlayerRtsNet target)
    {
        foreach (PlayerRtsNet unit in selectUnits)
    ...
    static public void  unitDied(PlayerRtsNet deadOne)
    {
        foreach (PlayerRtsNet player in allUnits)
    ...
    void PlayersLogic()
    {
        ...
        foreach (PlayerRtsNet player in allUnits) {
            ...
            if (player.state == PlayerRtsNet.state_t.WALLKING) {
                player.CheckArrived();
                return;
            }
            p = player.ClosestEnemy();
            if (player.state == PlayerRtsNet.state_t.IDLLE && p)
                player.setTarget(p); // STAT TO ATTAK ; ALLERT EMOTE
            
                // && Vector3.Distance(player.transform.position, enemy.transform.position) < 2
            if (player.state == PlayerRtsNet.state_t.ATTAKING) {
                inRange = false;
                foreach (PlayerRtsNet enemy in player._enemyInView)
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0)) {
            ...
            if (Physics.Raycast(ray, out hit, 250, UnitLayer)) {
                p = hit.collider.gameObject.GetComponent<PlayerRtsNet>();
        ...
        if (Input.GetMouseButtonUp(0)) {
            ...
            foreach (PlayerRtsNet players in allUnits)
